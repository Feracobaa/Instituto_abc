...
Deno.serve(async (req: Request) => { // eslint-disable-line
  ...
  if (callerError || !callerUser) return json(req, { error: "Unauthorized." }, 401);
  ...
  if (providerError || !providerRow || providerRow.role !== "owner")
    return json(req, { error: "Forbidden: only Etymon owners can create users." }, 403);
