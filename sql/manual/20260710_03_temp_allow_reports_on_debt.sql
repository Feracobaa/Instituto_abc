-- Migration: Add temporary allowance for report cards download with debt
-- Date: 2026-07-10
...
ADD COLUMN IF NOT EXISTS allow_reports_on_debt_temp BOOLEAN NOT NULL DEFAULT FALSE;

COMMENT ON COLUMN public.institution_settings.allow_reports_on_debt_temp IS 
'Temporarily allow report card downloads for guardians even if they have pending tuition debts.';

-- 2. Update get_student_report_snapshot function to respect temporary allowance
...
-- 4. Enable RLS Update Policy for Rectors on institution_settings
DROP POLICY IF EXISTS "Permitir actualización de configuraciones a Rectores" ON public.institution_settings;
CREATE POLICY "Permitir actualización de configuraciones a Rectores" 
