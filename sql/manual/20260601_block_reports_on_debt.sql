...
ADD COLUMN IF NOT EXISTS block_reports_on_debt BOOLEAN NOT NULL DEFAULT TRUE;

-- Update existing records to have it enabled by default
UPDATE public.institution_settings SET block_reports_on_debt = TRUE WHERE block_reports_on_debt = FALSE;
...
BEGIN
  ...
  IF NOT public.is_user_rector() AND NOT exists (
    ...
    BEGIN
      ...
      select coalesce(block_reports_on_debt, true)
