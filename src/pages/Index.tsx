...
function StaffDashboard() {
  ...
  const { data: gradeRecords } = useGradeRecords(
    activePeriod ? { periodId: activePeriod.id } : undefined
  );
  const { data: allGradeRecords } = useGradeRecords();
  const { data: schedules } = useSchedules();
  const { data: grades } = useGrades();

  const isLoading = teachersLoading || studentsLoading;
  const { data: instStatus } = useInstitutionStatus({ enabled: isRector });
  ...
  const studentAverages = students?.map((student) => {
    const records = allGradeRecords?.filter((record) => record.student_id === student.id) || [];
    const average = records.length > 0
      ? records.reduce((accumulator, record) => accumulator + record.grade, 0) / records.length
      : 0;
  ...
  return (
    <div className="space-y-6">
      ...
      {isRector && instStatus?.days_remaining !== null && instStatus?.days_remaining !== undefined && instStatus.days_remaining <= 30 && instStatus.current_period_end && (
        <ContractStatusBanner 
          daysRemaining={instStatus.days_remaining} 
          periodEnd={instStatus.current_period_end} 
        />
      )}
