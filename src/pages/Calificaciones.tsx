...
const Calificaciones = () => {
  ...
  const handleInlineGradeChange = async (record: GradeRecord, newGrade: number) => {
    if (!ensureEditablePeriod() || !selectedPeriod) {
      return;
    }

    const existingPartials = gradeRecordPartialsByRecordId.get(record.id) ?? [];
    
    const updatedPartials = existingPartials.length <= 1
      ? [{
          activity_name: existingPartials[0]?.activity_name || "Actividad 1",
          achievements: existingPartials[0]?.achievements || "",
          comments: existingPartials[0]?.comments || "",
          grade: newGrade,
          partial_index: 1,
        }]
      : existingPartials.map((p, idx) => ({
          activity_name: p.activity_name || `Actividad ${idx + 1}`,
          achievements: p.achievements || "",
          comments: p.comments || "",
          grade: idx === 0 ? newGrade : (typeof p.grade === "number" ? p.grade : 3.0),
    ...
    <MainLayout>
      <div className="space-y-6">
        ...
        <CalificacionesFilters
          ...
          onToggleInlineEdit={setInlineEditActive}
          canEdit={canManageCurrentPeriod}
        />
        ...
        {!selectedGrade || !selectedPeriod ? (
          ...
          <CalificacionesTable
            ...
            inlineEditActive={inlineEditActive}
            onInlineGradeChange={handleInlineGradeChange}
          />
        )}
