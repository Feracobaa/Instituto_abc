...
export default function Familias() {
  ...
  }, [studentsQuery.data, studentsQuery]);
  ...
  const handleResetPassword = async (studentId: string) => {
    try {
      const result = await resetGuardianPassword.mutateAsync(studentId);
      setLastProvisioned([
        {
          status: "created",
        ...
        {createdCredentials.length > 0 && (
          <div className="rounded-xl border border-emerald-500/25 bg-emerald-500/10 p-4 shadow-card">
            ...
            <div className="space-y-2">
              {createdCredentials.map((item, index) => (
                <div key={item.studentId || index} className="rounded-lg bg-background/70 p-3 text-sm">
                ...
                <div key={student.id} className="rounded-xl border bg-card p-4 shadow-card">
                  ...
                  {account ? (
                    <div className="mt-4">
                      <Button
                        variant="outline"
                        className="gap-2 border-amber-500/30 text-amber-500 hover:bg-amber-500/10 hover:text-amber-500"
                        onClick={() => handleResetPassword(student.id)}
                        disabled={resetGuardianPassword.isPending}
                      >
                        {resetGuardianPassword.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <KeyRound className="h-4 w-4" />}
                        Restablecer clave
                      </Button>
                    </div>
                  ) : (
