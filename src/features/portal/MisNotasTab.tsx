...
import { BlockedGradesAlert } from "./BlockedGradesAlert";
...
export default function MisNotasTab() {
  ...
  const hasPendingDebt = useMemo(() => {
    ...
    if (settings?.allow_reports_on_debt_temp) return false;
    return tuitionRecords.some((r) => (r.pending_amount ?? 0) > 0);
  }, [settings?.block_reports_on_debt, settings?.allow_reports_on_debt_temp, tuitionRecords]);
