...
interface CalificacionesFiltersProps {
  ...
  onToggleInlineEdit: (active: boolean) => void;
  canEdit: boolean;
}
      ...
      {canEdit && (
        <div className="flex h-10 items-center gap-2.5 rounded-lg border bg-background px-3">
          <Switch
            ...
            checked={inlineEditActive}
            onCheckedChange={onToggleInlineEdit}
          />
          <Label htmlFor="inline-edit-toggle" className="text-xs font-semibold cursor-pointer select-none">
            Edición rápida
          </Label>
        </div>
      )}
