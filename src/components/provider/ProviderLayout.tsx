    ...
    <div className={cn("etymon-shell min-h-screen w-full", theme === "light" ? "etymon-theme-light" : "etymon-theme-dark")}>
      <div className="grid min-h-screen grid-cols-1 lg:grid-cols-[260px_minmax(0,1fr)]">
        <aside className="hidden border-r [border-color:var(--et-border)] [background:var(--et-panel-bg)] lg:flex lg:flex-col">
          <div className="border-b [border-color:var(--et-border)] px-5 py-4">
            <div className="flex items-center gap-3">
              <div className="flex h-9 w-9 overflow-hidden rounded-lg bg-transparent p-0.5 shadow-[0_0_15px_rgba(0,231,167,0.2)]">
                <img src="/etymon-logo.png" alt="ETYMON Logo" className="h-full w-full object-contain" />
              </div>
              <div>
                <p className="text-[9px] font-semibold uppercase tracking-[0.15em] text-[var(--et-text-muted)]">Consola Global</p>
                <h1 className="text-lg font-bold tracking-tight text-[var(--et-text)]">Etymon</h1>
              </div>
            </div>
          </div>
          ...
          <footer className="border-t [border-color:var(--et-border)] p-4">
            <div className="rounded-xl border [border-color:var(--et-border)] [background:var(--et-chip-bg)] p-3 shadow-sm">
              <div className="flex items-center gap-2.5">
                <div className="flex h-7 w-7 items-center justify-center rounded-full bg-[var(--et-accent-soft)] text-xs font-bold text-[var(--et-accent)]">
                  {userName.charAt(0).toUpperCase()}
                </div>
                <div className="min-w-0 flex-1">
                  <p className="truncate text-xs font-semibold text-[var(--et-text)]">{userName}</p>
                  <p className="text-[10px] font-medium text-[var(--et-text-muted)]">Administrador Global</p>
                </div>
              </div>
            </div>
            <button onClick={signOut} className="etymon-btn-ghost mt-3.5 w-full text-xs py-2 gap-1.5 hover:bg-red-500/5 hover:text-red-400 hover:border-red-500/20">
