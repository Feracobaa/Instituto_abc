...
import { Shield, GraduationCap, Users, Calculator } from "lucide-react";
  ...
  return (
    ...
    >
      {isRector ? (
        ...
        <Calculator className={size === "sm" ? "h-2.5 w-2.5" : "h-3 w-3"} />
      ) : isParent ? (
        <Users className={size === "sm" ? "h-2.5 w-2.5" : "h-3 w-3"} />
      ) : (
        <GraduationCap className={size === "sm" ? "h-2.5 w-2.5" : "h-3 w-3"} />
      )}
      {isRector ? "Rector" : isContable ? "Contable" : isParent ? "Estudiante" : "Profesor"}
    </span>
  );
}
